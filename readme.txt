...
notes: (I'm writing this 6 months after the fact, probably not 100% complete or correct)
	...
	- mysql tables can be created by executing code like this (see line btc.pl 71 partial example): like CaptchaQuest::BTCDataObj::BTCSysUser->new($self)->create_table({alter=>1}); ... do it ONCE with each of the data objects. (BTCSysUser, BTCSysAddressbook, BTCSysSMSJunk) .. there should be Core framework data objects kicking around to execute similar on to resolve any complaints the framework code may give.
